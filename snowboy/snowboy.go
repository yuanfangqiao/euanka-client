...
import (
	"errors"
	snowboydetect "euanka/snowboydetect"
	"io"
	"strconv"
	"strings"
	"time"
...
func (d *Detector) ReadAndDetect(data io.Reader, listener *Listener) error {
	d.initialize()
	bytes := make([]byte, 1920)
	for {
...
func (d *Detector) runDetection(data []byte) snowboyResult {
	...
	if result == snowboyResultSilence {
		...
		//dataElapseTime := len(data) / (numChannels * (bitDepth / 8) * sampleRate)
		d.silenceElapsed += time.Duration(dataElapseTime)
		//log.Printf("runDetection  snowboyResultSilence:%d, len:%d, numChannels:%d, bitDepth:%d, sampleRate:%d", dataElapseTime, len(data), numChannels, bitDepth, sampleRate)
	} else {
